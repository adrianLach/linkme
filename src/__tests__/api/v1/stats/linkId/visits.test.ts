...
import { GET } from "@/app/api/v1/stats/[linkId]/visits/route";
...
describe("GET /api/v1/stats/:linkId/visits", () => {
  it("returns 401 when unauthenticated", async () => {
    mockGetApiUser.mockResolvedValue(null);
    const res = await GET(makeRequest("http://localhost/api/v1/stats/link-1/visits"), ctx("link-1"));
  ...
  it("returns 404 when link does not exist", async () => {
    ...
    const res = await GET(makeRequest("http://localhost/api/v1/stats/missing/visits"), ctx("missing"));
  ...
  it("returns paginated visits", async () => {
    ...
    mockPrisma.visit.findMany.mockResolvedValue([
      { id: BigInt(1), ip: "1.2.3.4", country: "US", createdAt: new Date() },
    ] as never);
    mockPrisma.visit.count.mockResolvedValue(1 as never);

    const res = await GET(
      makeRequest("http://localhost/api/v1/stats/link-1/visits?page=1&limit=10"),
      ctx("link-1")
    );
